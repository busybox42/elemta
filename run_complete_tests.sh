#!/bin/bash
#
# Elemta Complete Test Runner Script
#
# This script runs the complete test suite against the Docker deployment.
# It provides comprehensive testing of all Elemta SMTP server functionality.
...
show_help() {
    echo "Elemta Complete Test Runner Script"
    echo ""
...
while [[ $# -gt 0 ]]; do
    case $1 in
        ...
        --test)
            ...
            ;;
        --list)
            python3 test_elemta_complete.py --list
            exit 0
            ;;
        --verbose)
            VERBOSE=true
...
if [ ! -f "test_elemta_complete.py" ]; then
    print_status $RED "❌ Test script not found: test_elemta_complete.py"
    exit 1
fi
...
# Build test command
TEST_CMD="python3 test_elemta_complete.py --host $HOST --port $PORT"
...
# Run tests
print_status $BLUE "🚀 Running Elemta complete tests..."
